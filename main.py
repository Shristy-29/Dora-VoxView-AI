...
def get_webcam_frame():
    ...
    # Clear buffer by reading several frames quickly
    for _ in range(10):
        if camera is not None:
            camera.read()
    ret, frame = camera.read()
...
with gr.Blocks() as demo:
    gr.HTML("""
    <style>
        body, html, .gradio-container, .gr-blocks, .gradio-interface, .gradio-app, .main, .container, .gr-box, .gr-panel, .gr-column, .gr-row {
    ...
    <h1 style='color: white; text-align: center; font-size: 4em; background: transparent;'>DORA AI ASSISTANT</h1>
""")
